...
struct GradeEntryUndo: Identifiable, Equatable {
    ...
    func restore(to subject: Subject, in context: ModelContext) -> Bool {
        // Gefragt wird der Store, nicht das Objekt: `isDeleted` trägt nur, solange
        // die Löschung noch nicht gespeichert ist — danach steht das Fach als
        // freistehendes Objekt da, mit `isDeleted == false` und einer Beziehung,
        // in der die gelöschten Halbjahre noch stehen können. Ein Zähler über die
        // stabile Kennung antwortet in beiden Fällen richtig.
        guard SubjectDeletion.exists(subject, in: context) else { return false }

        // Und auch bei bestehendem Fach muss das Halbjahr noch da sein: es kann
        // ungespeichert gelöscht sein, während der Hinweisstreifen noch steht.
