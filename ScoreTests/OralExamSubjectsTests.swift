...
struct OralExamSubjectsTests {
    ...
    ///
    /// Drei Leistungsfächer, zwei Kernfächer, drei Basisfächer — genug, um die
    ...
    @Test("Kern- und Basisfächer stehen zur Wahl")
    func mandatoryAndOptionalSubjectsAreOptions() throws {
        ...
        // Ein Kernfach wird ohnehin vollständig eingebracht — die Wahl ändert
...
struct OnboardingOralExamTests {
    ...
    /// Ein Modell, das bis zu den Basisfächern durchgelaufen ist.
    private func model() -> OnboardingViewModel {
        let model = OnboardingViewModel()
        model.advancedSubjects = ["Deutsch", "Mathematik", "Biologie"]
        model.coreSubjects = ["Englisch", "Geschichte"]
        model.basicSubjects = ["Sport", "Musik"]
        return model
    }

    @Test("Zur Wahl stehen die Kern- und Basisfächer")
    ...
    func stalePicksAreDroppedOnTheWayIn() {
        ...
        model.basicSubjects = ["Sport"]

        model.step = .basicSubjects
        model.advance()
