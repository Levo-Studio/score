...
/// Basisfächer stehen — man wählt aus ihnen. Und in der Fächerliste, weil die
...
struct OralExamSubjectSelection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if options.isEmpty {
                Text("Du hast noch kein Fach, das sich mündlich prüfen liesse. Leg zuerst deine Basisfächer an.")
                    .font(.optionMeta)
                    .lineSpacing(4.5)
                    .foregroundStyle(ScorePalette.inkSecondary)
...
enum OralExamSubjects {
    ...
    /// geprüft. Ein Kernfach steht mit dabei — Deutsch und Mathematik *müssen*
        ...
        subjects
            ...
            .map { subject in
                OralExamSubjectSelection.Option(
                    ...
                    name: subject.name,
                    color: subject.color,
                    isAlreadyMandatory: subject.kind == .kernfach
                )
            }
    }
